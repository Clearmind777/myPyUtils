...
class PairMatcher:
	...
	def __init__(self, s:str='', bl:str='(', br:str=')')->None:	# 双下划线开头来定义私有方法
		if not (isinstance(s, str) and isinstance(bl, str) and isinstance(br, str)): raise TypeError
		if (not s) or (not bl) or (not br): raise ValueError
		self.s = s
		self.bl = bl
		self.br = br
	...
	def getChIdx(self, s:str, ch:str)->list:
		...
		while True:
			if self.isOneCh(ch): 
				idx = s.find(ch, idx + 1)
			else: idx = s.find(ch, idx + self.ofst)
			if idx == -1: break
			idxs.append(idx)
		return idxs
	...
	def getPair(self, s:str=None, bl:str=None, br:str=None, out:Literal['default', 'sorted']='default')-> Union[list[tuple], list[int]]:
		...
		if bl == br: 
			...
			if len(idxs) % 2 == 1 or len(idxs) == 0: print('ERR: Upaired Braket.'); exit(1)	# checkPaired()
